...
function parseObjectInterfaces(
  ...
  const interfaces: NativeModuleObjectSpec[] = [];
  const seen = new Set<string>();
...
function readManifest(root: string): LynxLibJson {
  ...
  if (android !== undefined) {
    ...
    normalizedPlatforms.android = {
      ...
      nodeApi: hasNodeApiAddons(android),
    };
  }
  ...
  if (harmony !== undefined) {
    normalizedPlatforms.harmony = {
      ...
      nodeApi: hasNodeApiAddons(harmony),
    };
  }
...
function hasNodeApiAddons(platform: Record<string, unknown>): boolean {
...
/**
 * Generates the Lynxtron entry that registers all shared NAPI creators.
 */
